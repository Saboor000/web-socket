...
export default function Sidebar({ users, currentUser }: SidebarProps) {
  return (
    <div className="glass flex h-full w-75 flex-col border-r border-white/10 md:flex">
          ...
          {users.map((user, idx) => (
            <div
              ...
              className={`flex items-center gap-3 rounded-lg px-3 py-2 transition-colors ${
                user === currentUser ? "bg-white/5" : "hover:bg-white/5"
              }`}
            >
              <div className="h-8 w-8 rounded-full bg-linear-to-br from-zinc-700 to-zinc-800 flex items-center justify-center text-sm font-medium border border-white/5">
                {user[0]?.toUpperCase()}
              </div>
              <span
                className={`text-sm ${
